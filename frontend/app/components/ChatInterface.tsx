...
export default function ChatInterface() {
  ...
  return (
    <div className="flex flex-col h-full max-w-4xl mx-auto" style={{width: '-webkit-fill-available'}}>
        ...
        {messages.map((message, index) => (
          ...
          >
            ...
            >
              <p>{message.content}</p>
              {message.source && (
                <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">Source: {message.source}</p>
              )}
            </div>
          </div>
        ))}
